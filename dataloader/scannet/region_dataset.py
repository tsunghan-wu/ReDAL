...
class RegionScannetDataset:
    ...
    def __init__(self, data_root, voxel_size, imageset='train', init_lst=None):
        ...
        if imageset == 'active-label':
            with open('dataloader/scannet/init_data/init_label_region.json') as f:
                json_dict = json.load(f)
            self.use_augs = {'scale': True, 'rotate': True, 'elastic': True, 'chromatic': True}
        elif imageset == 'active-ulabel':
            with open('dataloader/scannet/init_data/init_ulabel_region.json') as f:
                json_dict = json.load(f)
        ...
        if imageset in ['active-label', 'active-ulabel']:
            for k in json_dict:
                scan_id = k
                path = os.path.join(self.data_root, scan_id, 'coords.npy')
