...
class Tester:
    def __init__(self, args, active_iter, active_set, saver):
        self.args = args
        ...
        # prepare dataset
        self.dataset = active_set.pool_dataset
    ...
    def calculate_error(self):
        self.dataset.set_force_label(True)
        self.net.eval()
        if self.distributed is True:
        ...
        with torch.no_grad():
            for i_iter_test, batch in enumerate(loader):
                ...
                for batch_idx in range(self.batch_size):
                    ...
                    prob = torch.nn.functional.softmax(preds[cur_scene_pts][cur_inv], dim=1)
                    entropy = torch.mean(-prob * torch.log2(prob + 1e-12), dim=1).cpu().numpy()

                    target = all_labels.F[all_labels_C[:, -1] == batch_idx].cpu().numpy()
                    feats = inputs.F[cur_scene_pts][cur_inv]
                    feats = feats.cpu().detach().numpy()

                    self.saver.save_pred_error_entropy(feats[:, 3:], output, target, entropy, fname, self.active_iter)
                    idx += 1
...
if __name__ == '__main__':
    ...
    args.output_dir = os.path.join(args.model_save_dir, 'result')
    print(' '.join(sys.argv))
    print(args)
